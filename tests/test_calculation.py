...
def test_modulus_exact_division():
    calc = Calculation(operation="Modulus", operand1=Decimal("15"), operand2=Decimal("5"))
...
def test_integer_division_exact():
    calc = Calculation(operation="IntegerDivision", operand1=Decimal("15"), operand2=Decimal("5"))
    assert calc.result == Decimal("3")
...
def test_percentage_full():
    calc = Calculation(operation="Percentage", operand1=Decimal("100"), operand2=Decimal("100"))
    assert calc.result == Decimal("100")


def test_percentage_zero_numerator():
    calc = Calculation(operation="Percentage", operand1=Decimal("0"), operand2=Decimal("100"))
    assert calc.result == Decimal("0")


def test_percentage_zero_base():
    with pytest.raises(OperationError, match="Cannot calculate percentage of zero base value"):
...
def test_absolute_difference_equal_numbers():
    calc = Calculation(operation="AbsoluteDifference", operand1=Decimal("7"), operand2=Decimal("7"))
    assert calc.result == Decimal("0")
