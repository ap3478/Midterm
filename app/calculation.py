...
class Calculation:
    ...
    def calculate(self) -> Decimal:
        ...
        operations = {
            ...
            "IntegerDivision": lambda x, y: x // y if y!= 0 else self._raise_intdiv_zero(),
            "Percentage": lambda x, y: (x / y) * Decimal(100) if y!= 0 else self._raise_invalid_percentage(),
    ...
    @staticmethod
    def _raise_modulus_zero():  # pragma: no cover
        """
        Helper method to raise modulus by zero error.

        This method is called when a modulus by zero is attempted.
        """
        raise OperationError("Modulus by zero is not undefined")
    
    @staticmethod
    def _raise_intdiv_zero():  # pragma: no cover
        """
        Helper method to raise Integer division by zero error.

        This method is called when a Integer division by zero is attempted.
        """
        raise OperationError("Integer Division by zero is not allowed")
    
    @staticmethod
    def _raise_invalid_percentage():  # pragma: no cover
        """
        Helper method to raise division by zero error.

        This method is called when a division by zero is attempted.
        """
        raise OperationError("Cannot calculate percentage of zero base value")
